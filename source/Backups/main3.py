...
def register_user(email, password, role="user"):
    ...
    st.session_state.user_roles[email] = role
...
def get_current_user_role():
    ...
    if st.session_state.current_user:
        return st.session_state.user_roles.get(st.session_state.current_user, "user")
    return None
...
def get_risk_assessment_binary(probability, actual_target=None):
    ...
    if actual_target == 0:
        ...
        return "🚨 RISCO", "#DC2626", "Risco Detectado - Ação Imediata Requerida", "risco"
    
    else:
        if probability >= 0.5:
            return "🚨 RISCO", "#DC2626", "Alta Probabilidade de Risco - Revisão Requerida", "risco"
        else:
            return "✅ NORMAL", "#10B981", "Transação Aprovada - Baixo Risco", "normal"
...
def show_login_page():
    ...
    st.markdown("""
        <div style='background: linear-gradient(135deg, #1e3c72, #2a5298); padding: 40px; border-radius: 10px; margin-bottom: 20px;'>
            <h1 style='color: white; text-align: center; margin: 0; font-size: 2.5em;'>🛡️ Fraud Intelligence Platform</h1>
            <p style='color: #e0e0e0; text-align: center; font-size: 1.2em; margin: 10px 0 0 0;'>
    ...
    with tab2:
        ...
        register_role = st.selectbox("Tipo de Usuário", 
                                   ["user", "gestor", "admin"], 
                                   format_func=lambda x: {
                                       "user": "Usuário Normal", 
                                       "gestor": "Gestor de Services", 
                                       "admin": "Administrador"
                                   }[x])
        
        if st.button("Registrar", type="secondary", use_container_width=True):
            if register_email and register_password:
                success, message = register_user(register_email, register_password, register_role)
                if success:
                    st.success(message)
...
def show_main_interface():
    ...
    st.markdown(f"""
        <div style='background: linear-gradient(135deg, #1e3c72, #2a5298); padding: 20px; border-radius: 10px; margin-bottom: 20px;'>
            <div style='display: flex; justify-content: space-between; align-items: center;'>
                <div>
                    <h1 style='color: white; margin: 0; font-size: 2em;'>🛡️ Fraud Intelligence Platform</h1>
                    <p style='color: #e0e0e0; margin: 5px 0 0 0; font-size: 1em;'>
                    Sistema de detecção de risco em tempo real
                    </p>
                </div>
                ...
                with st.spinner("Analyzing transaction..."):
                    ...
                    if len(row) == 0:
                        ...
                        if actual_target is not None:
                            target_info = f" | Actual Target: {actual_target} ({'Risco' if actual_target == 1 else 'Normal'})"
    ...
    with tab3:
        st.markdown("<h2 style='font-size: 1.5em;'>Risco Probability Distribution</h2>", unsafe_allow_html=True)
        ...
        if st.button("Generate Distribution Analysis", type="primary"):
            with st.spinner("Analyzing data distribution..."):
                ...
                ax.axvspan(threshold, 100, alpha=0.2, color='#DC2626', label='Risco')
                
                ax.hist(proba_all, bins=20, alpha=0.7, color='#4B5563', edgecolor='black')
                ax.set_xlabel('Risco Probability (%)', fontsize=10)
                ax.set_ylabel('Frequency', fontsize=10)
                ax.set_title('Risco Probability Distribution - Binary Classification', fontsize=12)
                ...
                with col1:
                    ...
                    st.metric("🚨 Risco Transactions", f"{fraud_count} ({fraud_count/total*100:.1f}%)")
    ...
    # =============================================================================
    # TAB 4: BATCH PREDICTION (MANTIDO IGUAL)
    # =============================================================================
    ...
    # =============================================================================
    # PAINEL PRINCIPAL (MANTIDO IGUAL)
    # =============================================================================
    ...
    metric_config = [
        ...
        {"title": "Risco", "value": metrics['fraud_tx'], "change": metrics['today_change']['fraud'], "color": "#DC2626"},
    ...
    with col4:
        ...
        investigations = [
            {"Case": "High Probability Risk Pattern", "Priority": "High", "Status": "Under Review"},
...
def main():
    ...
    if "admin@system.com" not in st.session_state.users:
        register_user("admin@system.com", "admin123", "admin")
